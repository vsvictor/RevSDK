...
public class RevOkHttpDownloader implements Downloader {
    ...
    private static OkHttpClient createOkHttpClient(File cacheDir, long maxSize) {
        return RevSDK.OkHttpCreate(Constants.DEFAULT_TIMEOUT_SEC, false, false);
    }

    private final Call.Factory client;
    private final Cache cache;

    public RevOkHttpDownloader(Context context) {
        this(defaultCacheDir(context));
    }

    public RevOkHttpDownloader(File cacheDir) {
        this(cacheDir, calculateDiskCacheSize(cacheDir));
    }

    public RevOkHttpDownloader(final Context context, final long maxSize) {
        this(defaultCacheDir(context), maxSize);
    }

    public RevOkHttpDownloader(File cacheDir, long maxSize) {
        this(createOkHttpClient(cacheDir, maxSize));
    }

    public RevOkHttpDownloader(OkHttpClient client) {
        this.client = client;
        this.cache = client.cache();
    }

    public RevOkHttpDownloader(Call.Factory client) {
        this.client = client;
        this.cache = null;
    }
